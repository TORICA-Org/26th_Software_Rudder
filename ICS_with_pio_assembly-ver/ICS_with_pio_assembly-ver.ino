 ...
 * @brief 近藤科学 ICSサーボ用 1ピン半二重通信 Arduinoスケッチ (RP2040 PIO アセンブラ版)
 * 
 * 変換基板やダイオード等の追加回路は一切不要！
 * SIG線をRP2040のGPIOピンに直結（レベル変換回路の3.3V側に直結）するだけで、
 * PIOが自動的にオープンドレインで駆動し、完全な送受信を実現します。
 */
...
constexpr long    ICS_BAUDRATE = 115200;  // ICS通信速度 (115200 bps, 625000, 1250000 など)

// IcsPio オブジェクトの生成
IcsPio krs(ICS_SIG_PIN, ICS_BAUDRATE);

void printHelp() {
  Serial.println(F("\n====== ICS Direct Communication (No Converter Board / Diode Required) ======"));
...
void setup() {
  ...
  // ICS通信ドライバの初期化 (ここで PIO のメモリロード・ステートマシン初期化が行われます)
  krs.begin();
